...
# Extract transfer value from the correct column
TRANSFER_FIELD=$(echo "$OUTPUT" | grep 'sender' | tail -n1 | awk '{print $7 " " $8}')

NUM=$(echo "$TRANSFER_FIELD" | awk '{print $1}')
UNIT=$(echo "$TRANSFER_FIELD" | awk '{print $2}')
